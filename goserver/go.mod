// The k8s.io/kubernetes version below is the same version image/Dockerfile
// pins its kube-scheduler binary to (K8S_VERSION). Bumping it is the
// expected update path -- see the top-level README's "Alternative
// backend" section -- but unlike the binary, it isn't just a version
// string: k8s.io/kubernetes's own go.mod replaces every k8s.io/<staging
// component> with a local path (./staging/src/k8s.io/<component>), and
// replace directives aren't transitive, so this module has to restate
// each one, pointing at the matching v0.<minor>.<patch> release instead.
// After bumping the version, regenerate the replace block below from
// https://github.com/kubernetes/kubernetes/blob/v<version>/go.mod's own
// `replace (...)` block: keep every `k8s.io/x => ./staging/...` line's
// module name, replace the target with `k8s.io/x v0.<minor>.<patch>`.
module schedsim-go
